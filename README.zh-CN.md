...
- Fork 这个仓库，然后 vibe 你自己的版本。这个项目应用了 harness engineering，定制和添加特性应该尽量保持直接、简单。
...
harness 入口在 `scripts/harness.sh`。不带参数时，运行当前仓库基线检查：docs checks、`swift test` 和 `swift build`。
...
- 运行 `zsh scripts/harness.sh smoke` 做确定性的本地 app smoke pass。
...
1. 发布可靠的单 agent macOS MVP
2. 强化权限审批和 jump-back 行为
3. 改进多会话处理
4. 逐步扩展更多 agent 和终端集成
